...
with col2:
    racao_dia_kg = st.number_input("Ração consumida por dia (kg)", min_value=0.0, value=55.0, 
step=1.0)
    custo_kg_racao = st.number_input("Custo do kg da ração (R$)", min_value=0.0, value=2.10, 
step=0.05)
st.header("2. Custos Adicionais e Vendas")
col3, col4 = st.columns(2)
with col3:
    outros_custos_mes = st.number_input("Outros custos mensais (luz, embalagem, etc) (R$)", 
min_value=0.0, value=300.0, step=10.0)
with col4:
    preco_duzia = st.number_input("Preço de venda da dúzia (R$)", min_value=0.0, value=12.00, 
step=0.50)
# Cálculos
